...
class Loudness(torch.nn.Module):
    def __init__(self, nbark=49):
        ...
        self.threshs = Parameter(interp(abs_thresh_power_16k, nbark).unsqueeze(0).unsqueeze(0), requires_grad=False)

        exp = 6 / (torch.tensor(centre_of_band_bark_16k) + 2.)
        self.exp = Parameter(exp.clamp(min=1., max=2.) ** 0.15 * zwicker_power, requires_grad=False)

    def total_audible(self, tensor, factor: float = 1.):
        mask = tensor > self.threshs * factor

        return (tensor * mask).sum(dim=2)

    def time_avg_audible(self, tensor, silent):
        mask = tensor > self.threshs * 100.
        mask = mask * (~silent.unsqueeze(2))

        return (tensor * mask).mean(dim=1)

    def forward(self, pow_dens):
        loudness = (2.0 * self.threshs) ** self.exp * ((0.5 + 0.5 * pow_dens / self.threshs) ** self.exp - 1)
        loudness[loudness <= self.threshs] = 0.

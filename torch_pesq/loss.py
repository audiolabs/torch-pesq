...
class PesqLoss(torch.nn.Module):
    ...
    def align_level(self, signal):
        ...
        power = (signal.abs() ** 2 / 512.0)[:, :, 10:96].mean(dim=(1, 2), keepdim=True)
    ...
    def mos(self, estim, clean):
        ...
        estim, clean = self.to_spec(estim).swapaxes(1, 2), self.to_spec(clean).swapaxes(
            1, 2
        )
        ...
        band_pow_ratio = (
            ((mean_estim_pow + 1000) / (mean_clean_pow + 1000))
            .unsqueeze(1)
        ...
        frame_pow_ratio = (self.loudness.total_audible(clean, 1) + 5e3) / (
            self.loudness.total_audible(estim, 1) + 5e3
        )
        ...
        asymm_scaling = ((estim + 50.0) / (clean + 50.0)) ** 1.2
